...
import { TextField, Backdrop, CircularProgress, FormControl, OutlinedInput, InputAdornment } from "@mui/material"
            ...
            <div className="flex w-full flex-col h-screen justify-center items-center">
                <div className="flex flex-col bg-[#1e1e1e] w-96 h-fit md:w-1/2 xl:w-1/3 mt-4 mb-4 py-16 justify-center items-center focus:shadow-md focus:shadow-slate-600 rounded-3xl">
                    ...
                    <div className="w-fit h-fit flex flex-col space-y-3 mt-14">
                        ...
                        }} onChange={(event)=>setEmail(event.target.value)} error={error} id='email-input' label='Your Email' sx={{width:300}}/>
                    </div>
                    ...
                    <button onClick={login} className='bg-gradient-to-br from-green-400 to-emerald-700 w-fit mt-9 h-fit px-5 py-3 text-5xl rounded-xl transition-all duration-200 hover:scale-110 hover:-translate-y-2 hover:opacity-80'>Login</button>
                </div>
            </div>
    ...
    return(
        <div className='flex flex-col h-screen pt-10 justify-center items-center'>
            ...
            <div className='flex flex-col w-screen px-9 max-w-xl mt-14'>
                ...
                <FormControl fullWidth size='medium' variant='outlined' id='email-input' error={error}>
                    <OutlinedInput
                    type='email'
                    id='email-input'
                    onKeyDown={(input)=>{
            ...
            <div className='flex flex-col w-screen px-9 max-w-xl mt-7'>
                ...
                <FormControl fullWidth size='medium' variant='outlined' id='email-input' error={error}>
                    ...
                    inputRef={(input)=>{setPasswordInput(input)}}
                    />
                </FormControl>
            </div>
            <button onClick={login} className='text-2xl mt-9 w-40 rounded-md py-4 bg-gradient-to-tr from-violet-400 via-sky-400 to-emerald-400 transition hover:scale-105'>Sign In</button>
            <Backdrop open={loading}>
                <div className='flex flex-col w-screen h-screen justify-center items-center'>
                    <CircularProgress size={100} thickness={1.5}/>
                </div>
