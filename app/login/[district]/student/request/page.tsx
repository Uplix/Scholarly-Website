...
export default function Page({ params }: { params: { district: string } }) {
  ...
  const [requestTime, setRequestTime]: any = React.useState({
    physical: [],
    virtual: [],
  });
  ...
  React.useEffect(() => {
    let theRequestSub: any[] = [];
    let theRequestTime: { physical: any[]; virtual: any[] } = {
      physical: [],
      virtual: [],
    };
    let theRequestGrade: any[] = [];
    let theRequestLocation: any[] = [];
    onValue(ref(db, params.district + "/requestInfo/"), (snapshot) => {
      ...
      if (snapshot.child("times").hasChild("physicalTimes")) {
        theRequestTime.physical = snapshot
          .child("times")
          .child("physicalTimes")
          .val()
          .split(",");
      ...
      if (snapshot.child("times").hasChild("videoTimes")) {
        theRequestTime.virtual = snapshot
          .child("times")
          .child("videoTimes")
          .val()
          .split(",");
    ...
    return (
      <FormControl error={subError} sx={{ m: 1, minWidth: 250 }}>
        <InputLabel htmlFor="subject-select">
          {selectedSub == "" ? "Subject" : selectedSub}
        </InputLabel>
        <Select
          native
          defaultValue=""
          id="subject-select"
          label={selectedSub == "" ? "Subject" : selectedSub}
          onChange={handleChange}
        >
          <option aria-label="None" value="" />
          {requestSub.map((object: { subj: string; classes: any[] }) => (
            <optgroup label={object.subj}>
              {object.classes.map((object2) => (
                <option key={object.subj + object2} value={object2}>
                  {object2}
                </option>
              ))}
            </optgroup>
  ...
  const Timing = () => {
    ...
    return (
      <FormControl error={timeError} sx={{ m: 1, minWidth: 250 }}>
        <InputLabel htmlFor="time-select">
        ...
        <Select
          ...
          defaultValue=""
          id="time-select"
          label={selectedTime == "" ? "Time" : selectedTime}
          onChange={handelChange}
        >
          <option aria-label="None" value="" />
          {requestTime.virtual == null ||
          requestTime.virtual == undefined ? null : (
            <optgroup label="Virtual Times">
              {requestTime.virtual.map((object: string) => (
                <option key={"virtual" + object} value={"virtual," + object}>
                  {object}
                </option>
              ))}
            </optgroup>
          )}
          {requestTime.physical == null ||
          requestTime.physical == undefined ? null : (
            <optgroup label="Physical Times">
              {requestTime.physical.map((object: string) => (
                <option key={"physical" + object} value={"physcial," + object}>
                  {object}
                </option>
              ))}
            </optgroup>
      ...
      return (
        <FormControl error={locationError} sx={{ m: 1, minWidth: 250 }}>
          ...
          <Select
            ...
            defaultValue=""
            id="location-select"
            label={selectedLocation == "" ? "Location" : selectedLocation}
            onChange={handleChange}
          >
            <option aria-label="None" value="" />
            {requestLocations.map((object: string) => (
  ...
  const Grading = () => {
    ...
    return (
      <FormControl error={gradeError} sx={{ m: 1, minWidth: 250 }}>
        <InputLabel htmlFor="grade-select">
          {selectedGrade == "" ? "Grade" : "Grade: " + selectedGrade}
        </InputLabel>
        <Select
          native
          defaultValue=""
          id="grade-select"
          label={selectedGrade == "" ? "Grade" : "Grade: " + selectedGrade}
          onChange={handleChange}
        >
          <option aria-label="None" value="" />
          {requestGrades.map((object: number) => (
            <option key={object} value={object}>
              {"Grade: " + object}
            </option>
          ))}
      ...
      <LocalizationProvider dateAdapter={AdapterMoment}>
        <DemoContainer components={["DatePicker"]}>
          <DateCalendar
            ...
            maxDate={moment().add(1, "M").add(2, "d")}
          />
        </DemoContainer>
  ...
  const submit = () => {
    let myTime = date.startOf("day");
    let theDate = new Date(myTime);
    ...
    if (
      ...
      if (current == undefined) {
        ...
        try {
          let pushRef = push(
            ref(db, params.district + "/sessions/" + auth.currentUser?.uid),
          );
          set(pushRef, {
  ...
  return (
    <div className="w-full h-fit pb-6 flex flex-col justify-center items-center">
      <text className="text-slate-50 text-6xl mt-6 text-center font-light font-sans">
        Request Help
      </text>
      ...
      <div className="mt-10 animate-jump-in ease-in">
        <TextField
          ...
          helperText={
            textError == "long"
              ? "Must be shorter that 80 characters"
              : "Must be at least 10 characters. Hopefully more :)"
          }
      ...
      <button
        onClick={submit}
        className="text-center text-4xl text-slate-50 mt-10 px-3 py-2 bg-gradient-to-br from-emerald-800 to-green-400 rounded-xl transition hover:scale-110 hover:-translate-y-2 hover:opacity-80"
      >
        Submit
      </button>
      <Dialog open={requestSent}>
        <div className="w-fit h-fit bg-[#121820] py-9 px-9 flex flex-col items-center">
          ...
          <h5 className="mt-2 text-base font-light text-opacity-75 text-center">
            Your request was posted! Make your way back to the schedule page to
            see your request.
          </h5>
          <Link
            href={"/login/" + params.district + "/student/schedule"}
            className="mt-7 py-1 px-5 bg-emerald-500 rounded-lg text-lg"
          >
            Go to your schedule
          </Link>
        </div>
