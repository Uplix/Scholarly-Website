...
export default function Settings({ params }: { params: { district: string } }) {
  ...
  const [modalClassName, setModalClassName] = React.useState(
    "bg-[#1e1e1e] items-center w-96 h-fit flex flex-col animate-jump-in animate-ease-in rounded-2xl",
  );
  ...
  React.useEffect(() => {
    ...
    if (auth.currentUser != undefined && auth.currentUser != null) {
      ...
      onValue(
        ref(
          db,
          params.district + "/users/" + auth.currentUser?.uid + "/ratings",
        ),
        (snapshot) => {
  ...
  const closeModal = () => {
    setModalClassName(
      "bg-[#1e1e1e] items-center w-96 h-fit flex flex-col animate-jump-out animate-ease-out rounded-2xl",
    );
    setTimeout(() => {
      setSure(null);
      setModalClassName(
  ...
  return (
    <div className="flex flex-col w-full h-fit items-center pt-6 pb-10">
      ...
      <text className="text-slate-50 text-5xl text-center w-fit h-fit mt-8 mb-10 animate-jump-in animate-ease-in">
      ...
      <div className="mt-10 animate-jump-in animate-ease-in">
        <Rating
          sx={{ fontSize: 65 }}
          defaultValue={
            rating.numRatings == 0 ? 0 : rating.rating / rating.numRatings
          }
          precision={0.1}
          readOnly
        />
      ...
      <button
        ...
        className="text-center p-3 items-center font-light rounded-2xl text-3xl mt-12 bg-gradient-to-br from-rose-700 to-red-500 w-fit h-fit transition hover:scale-110 hover:-translate-y-2 hover:opacity-80"
      >
        Delete Account
      </button>
      {sure != null ? (
        <Modal open onClose={closeModal}>
          <div className="flex w-full h-full justify-center items-center">
            <div className={modalClassName}>
              <text className="text-center text-4xl space-x-9 mt-12 px-6">
                {sure?.text}
              </text>
              {sure?.fix ? null : (
                <text className="text-red-600 font-bold text-3xl text-center mt-8 px-4 ">
                  This action cannot be undone
                </text>
              )}
              <button
                onClick={sure?.click}
                className="text-center p-3 items-center font-light rounded-2xl text-5xl mt-12 mb-8 bg-gradient-to-br from-rose-700 to-red-500 w-fit h-fit transition hover:scale-110 hover:-translate-y-2 hover:opacity-80"
              >
                Confirm
              </button>
              <button
