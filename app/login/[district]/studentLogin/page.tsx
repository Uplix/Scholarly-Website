...
export default function StudentLogin({
  ...
  const [districtIconRef, setDistrictIconRef] = React.useState(
    process.env.NEXT_PUBLIC_DISTRICTS?.split(",")?.find(checker)?.split(":")[2],
  );

  React.useEffect(() => {
    setDistrictIconRef(
      process.env.NEXT_PUBLIC_DISTRICTS?.split(",")
        ?.find(checker)
        ?.split(":")[2],
    );
  });
  ...
  let signInEmail: undefined | string =
    process.env.NEXT_PUBLIC_DISTRICTS?.split(",")?.find(checker)?.split(":")[1];
  // alert(signInEmail)
    ...
    try {
      if (signInEmail != undefined && signInEmail != null) {
        ...
        if (session?.user?.email?.split("@")[1] == signInEmail) {
          ...
          throw new Error(
            "Please use your school Google account to login ending in " +
              signInEmail +
              ". Your email: " +
              session?.user?.email,
          );
    ...
    return (
      <div className="w-fit h-fit animate-jump-in animate-ease-in-out">
        <button
          disabled={loading}
          onClick={login}
          className="flex flex-row w-fit h-fit transition-all hover:scale-105 hover:-translate-y-0.5 bg-[#f2f2f2] hover:bg-[#e4e4e4] rounded-lg shadow-lg shadow-black items-center justify-center pr-3"
        >
          <Image
            alt="g-icon"
    ...
    <div className="flex flex-row min-h-screen w-screen justify-center items-center">
      <div className="w-full h-full flex flex-col max-w-3xl items-center justify-center">
        <div className="h-fit w-52 flex flex-row justify-center items-center">
          ...
          <Image
            alt="District-Logo"
            src={"/images/" + districtIconRef}
            width={70}
            height={70}
          />
        </div>
        <h4 className="text-4xl text-center mt-6 font-light px-5">
      ...
      <div className="flex-grow" />
      {windows >= 1024 ? (
        <div className="flex w-1/2 relative h-screen">
          <div className="absolute w-full h-full overflow-auto">
            <Image
              className="object-cover w-full h-full"
              alt="vertical-kids"
              fill={true}
              src={"/images/verticalKidsLogin.jpeg"}
            />
          </div>
        </div>
      ) : null}
