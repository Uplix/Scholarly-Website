...
export default function StudentLayout({
  ...
  const [menuClassName, setMenuClassName] = React.useState(
    "absolute left-0 top-0 flex flex-col w-96 min-h-screen h-fit bg-gradient-to-b from-emerald-500 to-indigo-400 animate-fade-right ease-in",
  );
  ...
  const closeMenu = () => {
    if (active) {
      setMenuClassName(
        "absolute left-0 top-0 flex flex-col w-96 min-h-screen h-fit bg-gradient-to-b from-emerald-500 to-indigo-400 animate-fade-right ease-out animate-alternate-reverse",
      );
      setActive(false);
      setTimeout(() => {
        setMenu(false);
  ...
  const TheMenu = () => {
    return (
      ...
      <div className={menuClassName}>
        ...
        <Link
          href={"/login/staff/options"}
          onClick={() => {
            setTimeout(() => closeMenu(), 50);
          }}
          className="self-center text-center text-white text-5xl mt-16 transition ease-in-out delay-150 hover:scale-110 hover:-translate-y-1 mb-5"
        >
          Options
        </Link>
      </div>
    ...
    <>
      <div className="flex flex-col h-full min-h-screen w-full">
        <div className="flex flex-row items-center h-20 w-screen bg-gradient-to-r from-slate-700 to-gray-900 border-b-2 border-slate-400">
          {!active ? (
            <button
              className="ml-8 transition ease-in-out hover:scale-110 hover:-translate-y-0.5"
              onClick={menuClick}
            >
          ...
          {loadingAvatar ? (
            ...
            <div className="scale-125 rounded-full mr-12 border-2 border-slate-100">
              <Avatar
                sx={{ bgcolor: stringToColor(auth.currentUser?.displayName) }}
              >
                {auth.currentUser == undefined || auth.currentUser == null
                  ? "NA"
                  : auth.currentUser?.displayName?.charAt(0) +
                    auth.currentUser?.displayName?.split(" ")[1].charAt(0)}
              </Avatar>
            </div>
          )}
        </div>
      ...
      <Modal open={menu} onClose={closeMenu}>
        <div className="w-fit h-fit">{menu ? <TheMenu /> : null}</div>
      </Modal>
    </>
  );
}
