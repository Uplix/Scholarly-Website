...
export default function Students() {
  ...
  React.useEffect(() => {
    ...
    const q = query(
      ...
      orderByChild("name"),
      startAt(
        search == undefined || search == null || search == "" ? "A" : search,
      ),
      limitToFirst(5),
    );
    // setLoading(true)
    ...
    onValue(q, (snapshot) => {
      snapshot.forEach((user) => {
        ...
        const data: rowType = {
          ...
          isTutor: user.child("isTutor").exists()
            ? user.child("isTutor").val()
            : false,
        };
  ...
  const searchForStudent = () => {
    if (search != undefined && search != null && search != "") {
      let theSearch: any = search.split(" ");
      for (let i = 0; i < theSearch.length; i++) {
  ...
  const studentClicked: GridEventListener<"rowDoubleClick"> = (
    params,
    event,
    details,
  ) => {
