...
export default function TimeEdit() {
  ...
  const TimeHead = ({ value, display }: { value: string; display: string }) => {
    ...
    const addIndiv = () => {
      ...
      if (theCurrent[value].length != 0) {
        if (
          theCurrent[value][theCurrent[value].length - 1] == undefined ||
          theCurrent[value][theCurrent[value].length - 1] == null ||
          theCurrent[value][theCurrent[value].length - 1] == ""
        ) {
          setTextError(true);
      ...
      <>
        <div>
          <div className="flex flex-row justify-start">
            ...
            <text className="text-4xl text-slate-100 font-medium mb-2">
              {display}
            </text>
            <button
              onClick={addIndiv}
              className="ml-4 transition hover:scale-110 hover:-translate-y-1"
            >
          ...
          <div className="flex flex-col ml-6">
            {times[value].map((theValue: string, theIndex: number) => (
              <IndivClasses
                value={theValue}
                index={theIndex}
                bigIndex={value}
  ...
  return (
    <div className="flex flex-col w-full h-full items-center pb-12">
      <text className="text-5xl text-slate-100 font-semibold mt-10">
        Edit Times
      </text>
