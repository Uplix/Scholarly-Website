...
export default function SubjectEdit(){
    ...
    const SubjectHead = ({value, index, array}:{value:any, index:number, array:any[]}) => {
        ...
        const addIndiv = () =>{
            ...
            if(theCurrent[index].classes.length != 0){
                if(theCurrent[index].classes[theCurrent[index].classes.length - 1] == undefined || theCurrent[index].classes[theCurrent[index].classes.length - 1] == null || theCurrent[index].classes[theCurrent[index].classes.length - 1] == ''){
                    setTextError(true);
            ...
            <>
                <div>
                    <div className='flex flex-row justify-start'>
                        ...
                        <button onClick={addIndiv} className='ml-2 transition hover:scale-110 hover:-translate-y-1'>
    ...
    const IndivClasses = ({value, index, bigArray, bigIndex}:{value:string, index:number, bigArray:any[], bigIndex:number}) => {
        ...
        return(
            <div className='flex flex-row items-end'>
                ...
                <TextField value={currentValue} onChange={(event)=>setCurrentValue(event.target.value)} onBlur={()=>{
                    let theCurrent = subjects;
    ...
    const addSubjcet = () => {
        console.log(subjects)
        if(subjects.length != 0 && subjects[subjects.length - 1].subject == undefined || subjects[subjects.length - 1].subject == null || subjects[subjects.length - 1].subject == ''){
    ...
    return(
        <div className="flex flex-col w-full h-full items-center pb-12">
            <text className='text-5xl text-slate-100 font-semibold mt-8'>Edit Subjects</text>
            <div className="flex flex-col w-full h-full mt-12 items-center">
                <div className='flex flex-row w-full h-fit justify-center px-12 flex-wrap gap-y-5'>
                    <Button color={addSubjcetButtonColor} onClick={addSubjcet} variant="outlined" className="w-52 h-18 text-2xl">Add Subject</Button>
