...
export function HomeNavigation({ width }: { width: number }) {
  ...
  const LoginButton = () => {
    return (
      <Link
        className="flex flex-row  mr-14 text-4xl transition ease-in-out hover:scale-105"
        href={"/login"}
      >
        {signedIn ? (
          <>
          ...
          <>
            <Person
              fontSize="large"
              className="mr-6 mt-1 border-2 border-slate-200 rounded-full p-0.5 scale-125"
            />
            <h4>Login</h4>
          </>
        )}
    ...
    return (
      <div className="w-fit h-fit animate-spin animate-once animate-ease-out">
        <button
          ...
          className="transition ease-in-out hover:scale-110 hover:-translate-y-0.5"
        >
    ...
    return (
      <div className="w-fit h-fit animate-rotate-y animate-once animate-ease-out">
        <button
          ...
          className="transition ease-in-out hover:-translate-y-0.5 hover:scale-110"
        >
          <Menu sx={{ fontSize: 40 }} />
        </button>
      </div>
  ...
  return (
    <div className="w-screen h-fit ">
      {width >= 1024 ? (
        <div className="w-0 h-0 lg:w-screen lg:h-fit invisible lg:visible">
          <div className="flex flex-row items-center w-full h-24 bg-blend-lighten bg-transparent shadow-lg shadow-sky-500">
            ...
            <div className="space-x-16 ml-20 h-full flex flex-row items-center justify-self-center">
              <Link
                className="text-4xl transition ease-in-out hover:scale-105 font-light"
                href={"/go/about"}
              >
                About
              </Link>
      ...
      {width < 1024 ? (
        <div className="flex flex-col w-screen h-fit lg:invisible lg:w-0 lg:h-0">
          <div className="flex flex-row items-center w-full h-24 bg-blend-lighten bg-transparent shadow-lg shadow-sky-500">
            <ScholarlyIcon />
            <div className="ml-6">{open ? <CloseIcon /> : <MenuIcon />}</div>
          ...
          <div className="w-screen h-fit flex flex-col items-center">
            <Collapse in={open}>
              <div className="w-screen  pt-4 pb-6 h-fit flex flex-col items-center gap-y-5 bg-gradient-to-b from-sky-500 to-transparent">
                <Link
                  className="text-5xl w-fit md:text-6xl transition ease-in-out hover:scale-110 text-center"
                  href={"/go/about"}
                >
                  About
                </Link>
